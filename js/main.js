...
$(document).ready(function() {
    ...
    $(document).on("click", '#price400Btn', function(e) {
        $('#pcfpAction').val('https://epl.ro/q/bGofHSv6IArbsto75IvT1rMI236bfA==');
    });
    $(document).on("click", '#price1000Btn', function(e) {
        $('#pcfpAction').val('https://epl.ro/q/bGsv3xJL*XWuaqg32QKO8c6ufjfggw==');
    });
    // Credit Card Button
    $(document).on("click", '#project-contact-us-button-credit-card', function(e) {
        // $('#pcfpAction').val('https://epl.ro/q/bGmu7ovNCEwwzBkCfJ2sR1C0rxG0OQ==');
        if ($('#popupForm').val() == 1) {
            // console.log("222project-contact-formpopup");
            $('#crcPopup').removeClass('cstm-red-brdr');
            $('#grErrorMsgPopup').hide();
            // errorPopup = true;
            // $('#project-contact-form-popup').submit();

        } else {
        // errorPopup = false;
            // console.log("333project-contact-formpopup");
            // errorPopup = false;
            $('#crcPopup').addClass('cstm-red-brdr');
            $('#grErrorMsgPopup').show();
            e.preventDefault();
        }
        // console.log("333errorPopup", errorPopup);
        // return false;
        // e.preventDefault();

        // if (errorPopup) {
        //     console.log("111errorPopup", errorPopup);
        //     e.preventDefault();
        // // return;
        //     $.ajax({
        //         type: "POST",
        //         url: "email-templates/project-contact.php",
        //         data: $("#project-contact-form-popup").serialize(),
        //         success: function(result) {
        //             e.preventDefault();
        ...
        //                 $(this).val('');
        //             });
        //             $("#success-project-contact-form-popup").html(result);
        //             $("#success-project-contact-form-popup").fadeIn("slow");
        //             $('#success-project-contact-form-popup').delay(4000).fadeOut("slow");
        //             window.location.href = "https://epl.ro/q/OWiMV0UdLWWT7bXApEcoUmwz9s6oeg==";

        //         }
        //     });
        // }
    });
    // Credit Card Button Ends

    // Thank You Button
    $(document).on("click", '#project-contact-us-button-thank-you', function(e) {
        $('#pcfpAction').val('thank-you.html');

                    // e.preventDefault();

        // $('#project-contact-form-popup').attr('action', 'thank-you.html');
        if ($('#popupForm').val() == 1) {
            // console.log("222project-contact-formpopup");
            $('#crcPopup').removeClass('cstm-red-brdr');
            $('#grErrorMsgPopup').hide();
        ...
        // e.preventDefault();
            ...
            // console.log("222errorPopup", errorPopup);
            // $.ajax({
            //     type: "POST",
            //     url: "email-templates/project-contact.php",
            //     data: $("#project-contact-form-popup").serialize(),
    ...
    // Thank You Button Ends

    $(document).on("submit", '#project-contact-form-popup', function(e) {
        if ($('#popupForm').val() == 1) {
            console.log("222project-contact-formpopup");
            $('#crcPopup').removeClass('cstm-red-brdr');
            $('#grErrorMsgPopup').hide();
        ...
        e.preventDefault();
        console.log("55454545454preventDefault");
        ...
        $.ajax({
                ...
                success: function(result) {
                    ...
                    $("#success-project-contact-form-popup").fadeIn("slow");
                    $('#success-project-contact-form-popup').delay(4000).fadeOut("slow");
                    window.location.href = $('#pcfpAction').val();
                }
            });
    });
    ...
    $(document).on("click", '#project-contact-us-button', function(e) {
        ...
        $('#indexForm').val();
        if ($('#indexForm').val() == 1) {
            console.log("222project-contact-form");
            $('#crcIndex').removeClass('cstm-red-brdr');
            $('#grErrorMsg').hide();
        } else {
            console.log("333project-contact-form");
            e.preventDefault();
            error = false;
    ...
    function ValidationProjectPopupForm() {
        $('#project-contact-form-popup input[type=text]').each(function(index) {
            if (index == 0) {
                if ($(this).val() == null || $(this).val() == "") {
                    $("#project-contact-form-popup").find("input:eq(" + index + ")").addClass("required-error");
                    errorPopup = false;
                } else {
                    $("#project-contact-form-popup").find("input:eq(" + index + ")").removeClass("required-error");
                }
            } else if (index == 2) {
                if (!(/(.+)@(.+){2,}\.(.+){2,}/.test($(this).val()))) {
                    $("#project-contact-form-popup").find("input:eq(" + index + ")").addClass("required-error");
...
$(document).ready(function() {
    ...
    $("#modalContent").load("modal-form.html", function(){
        ...
        // Add JS for Date Time picker
    });
    $("a[title|='Programeaza si Plateste Consultatia Juridica Online']").addClass('popup-with-zoom-anim').attr('href', '#modal-popup2');
    $('.popup-with-zoom-anim').magnificPopup({
        type: 'inline',
        fixedContentPos: false,
        fixedBgPos: true,
    ...
    // $("a[title|='Programeaza si Plateste Consultatia Juridica Online']").on('click', function(e) {
    //     e.preventDefault();
    ...
    $(document).on("click", '#dateTimeInpIcon', function() {
        $('#form_datetime').data("DateTimePicker").toggle()
    });
...
// Add JS for Date Time picker Ends